...
local function loadSettings()
    -- exports['qbr-core']:Notify(Lang:t("notify.hud_settings_loaded"), "success")
    -- NOTIFY exports['qbr-core']:Notify(9, Lang:t("notify.hud_settings_loaded"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
    Wait(1000)
...
local function HandleSetupResource()
    local admincheck = lib.callback.await('ps-hud:isadmin', false)
...
RegisterNetEvent("hud:onplyloded", function()
    Wait(2000)
    HandleSetupResource()
    -- local hudSettings = GetResourceKvpString('hudSettings')
    -- if hudSettings then loadSettings(json.decode(hudSettings)) end
    loadSettings()
    PlayerData = RedEM:GetPlayerData()
end)

RegisterNetEvent("hud:onplylogout", function()
    PlayerData = {}
    admin = false
    SendAdminStatus()
end)
...
local function restartHud()
    ...
    Wait(500)
    -- exports['qbr-core']:Notify(Lang:t("notify.hud_start"), "success")
    print("HUD Is Now Started!")
    SendNUIMessage({
        action = 'menu',
        topic = 'restart',
    })
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    exports['qbr-core']:TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
--             -- exports['qbr-core']:Notify(Lang:t("notify.load_square_map"))
--             exports['qbr-core']:Notify(9, Lang:t("notify.load_square_map"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
--         end
...
--         Wait(1200)
--         if Menu.isMapNotifChecked then
--             -- exports['qbr-core']:Notify(Lang:t("notify.loaded_square_map"))
--             exports['qbr-core']:Notify(9, Lang:t("notify.loaded_square_map"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
--         end
...
--         end
--         if Menu.isMapNotifChecked then
--             exports['qbr-core']:Notify(Lang:t("notify.load_circle_map"))
--             exports['qbr-core']:Notify(9, Lang:t("notify.loaded_square_map"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
--         end
...
--         Wait(1200)
--         if Menu.isMapNotifChecked then
--             exports['qbr-core']:Notify(Lang:t("notify.loaded_circle_map"))
--         end
--     end
-- end)
...
--             -- exports['qbr-core']:Notify(Lang:t("notify.cinematic_on"))
--             exports['qbr-core']:Notify(9, Lang:t("notify.cinematic_on"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
--         end
--     else
--         CinematicShow(false)
--         if Menu.isCinematicNotifChecked then
--             -- exports['qbr-core']:Notify(Lang:t("notify.cinematic_off"), 'error')
--             exports['qbr-core']:Notify(9, Lang:t("notify.cinematic_off"), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
--         end
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        -- exports['qbr-core']:Notify(Lang:t("notify.engine_off"))
        print("Engine Shut Down!")
    else
        -- exports['qbr-core']:Notify(Lang:t("notify.engine_on"))
...
--                         exports['qbr-core']:Notify(Lang:t("notify.low_fuel"), "error")
...
CreateThread(function()
	...
	while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            local playerInVehcile = IsPedOnMount(PlayerPedId()) or IsPedOnVehicle(PlayerPedId())
            ...
            lastHeading = heading
            if lastIsOutCompassCheck ~= Menu.isOutCompassChecked and not IsPedOnMount(PlayerPedId()) or not IsPedOnVehicle(PlayerPedId()) then
                if not Menu.isOutCompassChecked then
                    SendNUIMessage ({
                        action = 'baseplate',
                        topic = 'opencompass',
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        print("triggered")
        TriggerServerEvent('hud:server:updatestats', GetPlayerServerId(PlayerId()))
    end
end)
